...
- **RabbitMQ.** Connection URL via `RABBITMQ_URL` (required) ([src/config.ts:52-55](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/config.ts#L52-L55)). Publishes to two durable topic exchanges:
  ...
  - `wiki.events` with routing key `wiki.update.<owner>.<repo>` — consumed by the wiki pipeline. Publishes for **all** merged PRs (not just package repos); the wiki updater self-filters un-onboarded repos ([src/rabbit.ts:11](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/rabbit.ts#L11), [src/rabbit.ts:154-180](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/rabbit.ts#L154-L180), [src/server.ts:147-173](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/server.ts#L147-L173)).
...
- **Central wiki pipeline.** Subscribes to `wiki.events`. Receives events for **all** merged PRs; self-filters un-onboarded repos (unknown-repo, not-allowlisted, no-existing-pages). Best-effort — failure is logged but does not fail the request ([src/server.ts:147-173](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/server.ts#L147-L173)).
