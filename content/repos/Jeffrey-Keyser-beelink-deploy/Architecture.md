...
**`src/rabbit.ts` — publisher.** Asserts two durable topic exchanges: `packages.events` and `wiki.events` ([src/rabbit.ts:10-11](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/rabbit.ts#L10-L11)). Publishes `package.push.<owner>.<repo>` and `wiki.update.<owner>.<repo>` routing keys with persistent messages ([src/rabbit.ts:108-156](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/rabbit.ts#L108-L156)). Auto-reconnects with exponential backoff capped at 60 s ([src/rabbit.ts:13-86](https://github.com/Jeffrey-Keyser/beelink-deploy/blob/main/src/rabbit.ts#L13-L86)).
